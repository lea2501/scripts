...
$su apt-get -y --fix-missing install usbutils udftools bash-completion htop findutils acpi cpufreqd lm-sensors ntp alsa-tools alsa-utils xdotool tree

# pulseaudio
$su apt-get -y --fix-missing install pulseaudio pavucontrol

# devel
$su apt-get -y --fix-missing install cmake openjdk-17-jdk openjdk-11-jdk openjdk-11-jdk-headless openjdk-11-jre maven gradle npm
$su apt-get -y --fix-missing install jq git subversion groovy podman mariadb-server mariadb-client geany geany-plugins #docker.io

# multimedia
$su apt-get -y --fix-missing install flac opus-tools vorbis-tools wavpack mpv ffmpeg sox shntool lsdvd libbluray-bdj

# extra tools
$su apt-get -y --fix-missing install moc lynx w3m newsboat rtorrent amule youtube-dl
...
# net
$su apt-get -y --fix-missing install curl axel tigervnc-viewer openconnect network-manager network-manager-openconnect samba x11vnc
