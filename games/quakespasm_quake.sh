...
# run
compiled_bin="$HOME/src/quakespasm/Quake/quakespasm"
if [ -x "$compiled_bin" ]; then
  quakespasm_bin="$compiled_bin"
else
  quakespasm_bin=$(command -v quakespasm 2>/dev/null) || {
    echo "ERROR: quakespasm executable not found" >&2
    exit 1
  }
fi
set -x
